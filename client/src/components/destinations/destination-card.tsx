...
export function DestinationCard({ destination, className }: DestinationCardProps) {
  return (
    <Link href={`/destinations/${destination.id}`} className={cn(
      "destination-card rounded-xl overflow-hidden bg-white shadow cursor-pointer transition-all duration-300 hover:shadow-lg hover:-translate-y-1 block",
      className
    )}>
